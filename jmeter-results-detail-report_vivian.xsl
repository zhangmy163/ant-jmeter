...
<xsl:param    name="titleReport" select="'JMeter Test Results'"/>
...
<xsl:template name="failure">
	...
	<table align="center" class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
		...
		<xsl:for-each select="/testResults/*[not(@tn = preceding::*/@tn)]">
			...
			<xsl:variable name="thread" select="@tn" />
				      ...
				      <img src="expand.png" alt="expand/collapse"><xsl:attribute name="id"><xsl:text/>page_details_<xsl:value-of select="position()" />_image</xsl:attribute></img>				      
                        ...
                        <tr class="page_details">
                           ...
                           <td colspan="8" bgcolor="#FF0000">
                              <div align="center">
			         <b>Details for Page "<xsl:value-of select="$thread" />"</b>
			         ...
			         <xsl:for-each select="../*[@tn = $thread]">			         			            
			            <tr>
			               <td align="left">
							...
							<td align="left" class="zebra"><xsl:value-of select="method"/><xsl:text> </xsl:text><xsl:value-of select="java.net.URL"/></td>							
			               <td align="left">
